...
- [X] T017 [US2] Extend `js/compose/dropdown.js` + `js/compose/insert.js` with the **second scope menu** (FR-012a) offering `whole` / `typedPortion` / `startToEndWord`; for `startToEndWord` capture the user-typed end word and slice the authentic verse from the typed start through it (FR-015). **Done** — `accept()` now freezes the citation span and opens the scope menu (`dropdown.showScope`) instead of inserting; `startToEndWord` opens a focused end-word input (`dropdown.showEndWord`). `insert.buildBody` Tier-1-aligns the typed fragment onto the verse (reusing `js/verifier/normalize.js`, newly added to `manifest.json` content_scripts) and slices the ORIGINAL verse words — inserted text is always authentic (FR-017). `focusout` is guarded so focusing the end-word input doesn't tear down the instance.
- [X] T018 [US2] Handle the end-word-not-found case in `js/compose/insert.js`: when the end word is absent from the matched verse after the start, surface the localized "end word not found" message and refuse the truncated insert (FR-016). **Done** — `buildInsertText` returns `{error:'endWordNotFound'}`; the orchestrator re-shows the prompt with the localized `ac_endword_not_found` note and makes no insertion.
- [X] T019 [P] [US2] Add fixtures `tests/fixtures/compose/scopes.{html,expected.json}` covering all three scopes plus the end-word-not-found path; assert inserted text per scope. **Done** — like T015, covered by in-page hosts in `tests/autocomplete_check.js` (separate fixture files unnecessary): asserts typedPortion (typed span, not whole verse), startToEndWord (start→unique end word, derived from the index per Principle I), and the FR-016 refusal.
- [X] T020 [US2] Run the gate; iterate until US2 acceptance scenarios pass; `npm test` green. **Done** — `autocomplete_check.js` 30/30 (US1 updated for the two-step accept→scope flow + 6 new US2 assertions); full suite green (60/60 + panel 20/20 + swap 137/137 + interaction 20/20).
...
- [X] T021 [US3] Extend `js/compose/match.js` to expose the **cascade** exact → word-level → fuzzy → none, carrying `tier` per candidate so the dropdown can show drift candidates (FR-007). **Done** — `match.js` is a thin pass-through, so the work was adding the deferred fuzzy tier to `matchPartial` in `js/background.js`: new `findFuzzyGlobal` reuses the verifier's `softWordIndexLookup` + `wordLevelCompareSingleAyahLoose` (mirrors `findAllGlobalMatches`'s partial branch — Principle V, no new matching logic), wired as Tier 3 after wordLevel and tier-first/mushaf ranked. `match.js` exposes the `tier` per candidate verbatim.
- [X] T022 [US3] In `js/compose/index.js`, when the cascade yields no candidate, set the citation's state to not-recognized so it is marked **red** (delegating the actual styling to `render-editable.js` from US4; minimal red mark if US4 not yet present) (FR-008). **Done** — `markNotRecognized()` records `lastClassification = {ref:null, verdict:'red', viaFallthrough:true}` (FR-011a) and applies a minimal red span in contenteditable via the new `QuranComposeEditable.markRange()` (reuses the existing `.quran-red` palette — Principle II, no new color); plain inputs carry no markup (FR-018b). `process()` now also resets any lingering scope/end-word instance state on a fresh candidate render.
- [X] T023 [P] [US3] Add fixtures `tests/fixtures/compose/cascade.{html,expected.json}` with a word-level-drift fragment, a fuzzy fragment, and a no-match fragment; assert offered tiers and the not-recognized state. **Done** — per the T015/T019 precedent, covered by in-page hosts in `tests/autocomplete_check.js` (separate fixture files unnecessary): a one-alef-drift interior word → wordLevel candidate (and NOT exact); a non-Quranic interior substitution → fuzzy candidate; gibberish → not-recognized red + minimal mark in contenteditable. Ground truth derived from the shipped index (Principle I); only the lead-in and deliberate drift/gibberish tokens are hand-typed.
- [X] T024 [US3] Run the gate; iterate until US3 acceptance scenarios pass; `npm test` green. **Done** — `autocomplete_check.js` **38/38** (30 prior + 8 US3); full suite green (main 60/60, panel 20/20, swap 137/137, interaction 20/20) — no regression.
...
- [X] T025 [US4] Create `js/compose/render-editable.js`: classify recognized citations via `js/verifier/classify.js` and apply the verdict color + Quran font; **persist markup in contenteditable** (FR-018b), skip styling in plain inputs; render **pre-existing citations on focus** (FR-018a); all gated by `prefs.autocomplete.liveRender` (FR-018). **Done** — `QuranComposeRenderEditable.mark()` ADDITIVELY wraps a citation span (`<span class="quran-ac-cite quran-<verdict>">` + Quran-font class/inline family) via `extractContents`+`insertNode`, guarded by a text-equality check so it can NEVER delete the user's text (standing rule). Verdict is derived from the matcher tier (exact→green / wordLevel→yellow / fuzzy|none→red — Principle V, reuse the verifier's decision rather than re-classify) reusing content.css's existing palette (Principle II, no new color). `index.js` renders the just-inserted ayah in contenteditable (`persistedMarkup`), skips inputs, and `renderOnFocus()` marks pre-existing citations (FR-018a), all gated by `liveRender`. Font family resolved via `QuranFonts.familyFor(prefs.font)` (added `js/render/fonts.js` to the gate bundle).
- [X] T026 [US4] In `js/compose/dropdown.js` + `js/compose/index.js`, implement **instance dismissal** (type-past / caret-away closes the dropdown, not the feature; no Esc) and route the unresolved citation to `render-editable.js` for **fall-through classification** (FR-011, FR-011a). **Done** — type-past already closes the instance via `process()` re-detection; `onFocusOut` now calls `dismissFallthrough()` (marks the unresolved citation by its top-candidate verdict and records `lastClassification.viaFallthrough`) before closing, and the no-candidate path in `process()` marks red (FR-008/011a). No Esc handling (FR-011) is preserved.
- [X] T027 [US4] Extend `html/options.html` + `js/options.js` with the **Autocomplete settings** section (enable, live-render, reference format, reference placement, min-word gate) wired to `PREFS_WRITE`, localized via `[data-i18n]`, reflecting live via `PREFS_CHANGED` (FR-019). **Done** — new "Ayah autocomplete" section with `#ac-enable`, `#ac-live-render`, `#ac-ref-format`, `#ac-ref-placement`, `#ac-min-words`, each `[data-i18n]`-localized (keys already shipped in T009) and patching `prefs.autocomplete` via `savePrefs`; the compose orchestrator reconciles live off `PREFS_CHANGED`.
- [X] T028 [P] [US4] Add fixtures `tests/fixtures/compose/render.{html,expected.json}` covering contenteditable persistent markup, plain-input text-only skip, pre-existing-on-focus, and fall-through verdict; assert `persistedMarkup` + `lastClassification` (SC-007). **Done** — per the T015/T019/T023 precedent, covered by in-page hosts in `tests/autocomplete_check.js` (separate fixture files unnecessary): asserts contenteditable `persistedMarkup` + verdict/Quran-font span, plain-input `persistedMarkup:false` (matching/insertion preserved), pre-existing-on-focus mark (text not rewritten), caret-away fall-through verdict, and the `liveRender:false` toggle skipping markup while still inserting.
- [X] T029 [US4] Run the gate; iterate until US4 acceptance scenarios pass; `npm test` green. **Done** — `autocomplete_check.js` **74/74** (15 new US4 assertions); full suite green (suite 60/60, i18n 167 keys ar/en parity, panel 20/20, swap 137/137, interaction 20/20) — no regression.
...
- [X] T030 [P] Performance sweep: confirm the min-word gate, debounce, and client-side narrowing keep typing responsive (no per-keystroke background round-trip), satisfying SC-005 on the fixture hosts. **Done** — verified the existing min-word gate + 100ms debounce already prevent per-keystroke round-trips (client-side filtering not added: it would risk diverging from the verifier's soft-tier ranking — Principle V). New gate assertion "SC-005 rapid typing burst coalesces to a single match query (debounced)" proves a 5-keystroke burst inside the debounce window fans out to exactly ONE `MATCH_PARTIAL`.
- [X] T031 [P] i18n parity check: every new `t()` key exists in both `ar` and `en` with no missing-translation fallback (SC-008). **Done** — `i18n_check` reports 167 keys in ar/en parity (incl. the `ac_*` dropdown/scope/options keys).
- [X] T032 [P] Porting-discipline pass (Principle V): read the advanced copy's autocomplete at `C:\Users\mosta\PycharmProjects\QuranChromePlugin`, confirm its cases (editor quirks, caret rects, IME, end-word truncation, multi-citation fields) are covered by fixtures, and confirm no implementation was ported verbatim; record notes. **Done** — notes recorded in [research.md](./research.md#t032--porting-discipline-pass-principle-v). Confirmed no verbatim port: the advanced copy uses a destructive `deleteContents()`+`insertNode` splice and bails on multi-node citations; our rebuild uses non-destructive `execCommand('insertText')`, handles split nodes, adds the scope menu / IME guard / persistent verdict rendering, and matches via the rebuilt verifier RPC. Only the case inventory informed the design.
- [X] T033 Final full-suite run: `npm test` (existing suite green — no regression) + `node tests/autocomplete_check.js` (all US1–US4 assertions); confirm SC-001…SC-008 hold simultaneously; stop-the-line on any regression. **Done** — suite 60/60, i18n 167 keys parity, panel 20/20, swap 137/137, interaction 20/20, autocomplete **75/75** (US1–US4 + SC-005 perf). Feature 003 complete.
