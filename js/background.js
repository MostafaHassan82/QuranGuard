...
// Equal distance does NOT mean equal alignment quality, so ties break in two
// stages:
//   1. More kept (soft-equal) words wins. A citation with words omitted
//      mid-phrase (e.g. "يجعلون أصابعهم من الصواعق حذر الموت" dropping
//      "في آذانهم") ties at d=2 between the full window — every cited word
//      keeps, two `missing` inserts — and a shorter shifted window that
//      "aligns" by substituting correct cited words (يجعلون أصابعهم → في
//      آذانهم). The keep count (6 vs 4) picks the full window, so the
//      suggested excerpt retains the words the user got right.
//   2. Then prefer the window whose length is closest to candT1.length: the
//      equal-length window aligns a changed word as a `sub` (one keep-row
//      swap) rather than letting a shorter window degrade it into an `extra`
//      (cited word with no authentic counterpart). Without this bias a
//      *trailing* substitution (e.g. "رب الكون" vs "رب العالمين") ties at
//      equal keeps between the L=candLen-1 window — which drops the last word
//      as `extra`, so the diff overlay shows a strike with no replacement —
//      and the L=candLen window that renders the proper `sub`. The
//      substitution window is the one the reader needs (it shows the correct
//      word).
function bestAlignWindow(candT1, ayahT1) {
  ...
  // Kept-word count of the same alignment the diff overlay will render.
  // Computed lazily — only when two windows tie on distance.
  const keepsOf = (s, L) => {
    const win = ayahT1.slice(s, s + L);
    return alignedWordDiff(candT1, candT1, win, win)
      .reduce((n, o) => n + (o.op === 'keep' ? 1 : 0), 0);
  };
  let best = null;
  for (let s = 0; s + minLen <= ayahT1.length; s++) {
    for (let L = minLen; L <= maxLen && s + L <= ayahT1.length; L++) {
      ...
      if (best === null || d < best.d) {
        ...
        if (best.k === null) best.k = keepsOf(best.s, best.L);
        const k = keepsOf(s, L);
        if (k > best.k || (k === best.k && lenGap(L) < lenGap(best.L))) {
          best = { s, L, d, k };
        }
      }
      if (best.d === 0 && lenGap(best.L) === 0) return best;
    }
  }
  return best;
}
